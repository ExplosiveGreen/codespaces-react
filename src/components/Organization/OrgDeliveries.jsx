...
function OrgDeliveries() {
  ...
  const ApproveDelivery = async (delivery) => {
    ...
    if (result) {
      ...
      const resultPromise = (typeof delivery.donations == 'object' ? [delivery.donations] : delivery.donations).map(async (donation) => {
  ...
  const columns = [
    ...
    { id: 'status', label: 'Status', accessor: (row) => row.status },
    {
      id: 'actions', label: 'Actions', accessor: (row) => {
        if (row.status == "Awaiting") return <Button onClick={async () => await ApproveDelivery(row)}>Approve</Button>
        return (<></>)
      }
    }
  ];
