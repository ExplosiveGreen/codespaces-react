...
function CarrierHome() {
  ...
  const updateDalivery = async (delivery) => {
    ...
    dispatch(addRoute({ lat: delivery.donator.address.latitude, lng: delivery.donator.address.longitude }))
    dispatch(addRoute({ lat: delivery.organization.location.latitude, lng: delivery.organization.location.longitude }))
    await getDonators();
  }
  ...
  const getDonators = async () => {
    ...
    const donatorList = (Array.isArray(Donatorresult) ? Donatorresult : []).filter((filter) => "address" in filter && filter.delivery_requests.length != 0)
    ...
    setDonators((await Promise.all(donatorPromise)).filter(d => d.delivery_requests.length != 0))
  }
  const getLocations = () => {
    if (Boolean(generate))
      return (Array.isArray(donators) ? donators : [])
        .map((donator) => {
          return {
            location: {
            ...
            element: (
              <>
                {(donator.delivery_requests || []).map((dr) => dr.status == "Pending" && (
                  <>
                    {dayjs.tz(dr.delivery_date, 'America/New_York').format('YYYY-MM-DD HH:MM')}
                    <Button onClick={async () => await updateDalivery(dr)}>Accept</Button>
                  </>
                ))}
              </>
            ),
    ...
    else return routes.map(route => { return {location: route}})
  };
  useEffect(() => {
    getDonators()
  }, []);
